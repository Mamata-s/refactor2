...
def evaluate_model_edges(opt):
    ...
    for file in os.listdir(opt.image_path):  
        ...
        label_path = os.path.join(opt.label_path, opt.dictionary[file])
        downsample_path = os.path.join(opt.downsample_path, file)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--label-path',type=str,required=True,help='path for label images',default='gaussian_dataset25_mul/z_axis/label/test')
    parser.add_argument('--image-path',type=str,required=True,help='path for lr image',default='')
    parser.add_argument('--annotation-path',type=str,required=False,help='path for lr and hr image annotation',default='gaussian_dataset25_mul/z_axis/factor_4/test_annotation.pkl')
    parser.add_argument('--downsample-path', type=str, metavar='',help='gaussian mult  downsample image',default='')
    ...
    if opt.annotation_path:
        opt.dictionary = read_dictionary(opt.annotation_path)
    else:
        dir_dict = create_dictionary(opt.image_path,opt.label_path)
        opt.dictionary = create_dictionary(opt.image_path,opt.downsample_path)
    ...
    if opt.edges:
        ...
        print(opt)
        print("Evaluating Model on Images")
        print(len(opt.dictionary))
        evaluate_model(opt) # evaluate model trained on image not edges
    # print(opt)
