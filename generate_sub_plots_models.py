...
def save_results(result_image_list,result_image_list_key,label_image,save_dir,save_name, scores):
    ...
    ax1.set_title(result_image_list_key[0], fontsize=fnsize)

    ax2 = plt.subplot(2,6,3)
    ax2.imshow(result_image_list[1][328:510, 224:406], cmap='gray')
    ax2.set_title(result_image_list_key[1],fontsize=fnsize)
    ax2.set_xlabel('({:.3f},{:.3f})'.format(scores[1][first_index], scores[1][second_index]), fontsize=fnsize1)

    ax3 = plt.subplot(2,6,4)
    ax3.imshow(result_image_list[2][328:510, 224:406], cmap='gray')
    ax3.set_title(result_image_list_key[2],fontsize=fnsize)
    ax3.set_xlabel('({:.3f},{:.3f})'.format(scores[2][first_index], scores[2][second_index]),fontsize=fnsize1)

    ax4 = plt.subplot(2,6,5)
    ax4.imshow(result_image_list[3][328:510, 224:406], cmap='gray')
    ax4.set_title(result_image_list_key[3],fontsize=fnsize)
    ax4.set_xlabel('({:.3f},{:.3f})'.format(scores[3][first_index], scores[3][second_index]),fontsize=fnsize1)

    ax5 = plt.subplot(2,6,6)
    ax5.imshow(result_image_list[4][328:510, 224:406], cmap='gray')
    ax5.set_title(result_image_list_key[4], fontsize=fnsize)
    ax5.set_xlabel('({:.3f},{:.3f})'.format(scores[4][first_index], scores[4][second_index]),fontsize=fnsize1)

    ax6 = plt.subplot(2,6,8)
    ax6.imshow(result_image_list[5][328:510, 224:406], cmap='gray')
    ax6.set_title(result_image_list_key[5], fontsize=fnsize)
    ax6.set_xlabel('({:.3f},{:.3f})'.format(scores[5][first_index], scores[5][second_index]),fontsize=fnsize1)

    ax7 = plt.subplot(2,6,9)
    ax7.imshow(result_image_list[6][328:510, 224:406], cmap='gray')
    ax7.set_title(result_image_list_key[6], fontsize=fnsize)
    ax7.set_xlabel('({:.3f},{:.3f})'.format(scores[6][first_index], scores[6][second_index]),fontsize=fnsize1)

    ax8 = plt.subplot(2,6,10)
    ax8.imshow(result_image_list[7][328:510, 224:406], cmap='gray')
    ax8.set_title(result_image_list_key[7], fontsize=fnsize)
    ax8.set_xlabel('({:.3f},{:.3f})'.format(scores[7][first_index], scores[7][second_index]),fontsize=fnsize1)

    ax9 = plt.subplot(2,6,11)
    ax9.imshow(result_image_list[8][328:510, 224:406], cmap='gray')
    ax9.set_title(result_image_list_key[8], fontsize=fnsize)
    ax9.set_xlabel('({:.3f},{:.3f})'.format(scores[8][first_index], scores[8][second_index]),fontsize=fnsize1)
    ...
    ax11.set_title('ground truth', fontsize=fnsize)
    rect = patches.Rectangle((230, 350), 182, 182, linewidth=1, edgecolor='r', facecolor='none')
    ax11.set_xlabel('(PSNR,NRMSE)',fontsize=fnsize1)

    # Add the patch to the Axes
    ax11.add_patch(rect)


    axes = [ax1, ax2, ax3, ax4, ax5, ax6, ax7, ax8, ax9,ax11]
    for ax in axes:
...
if __name__ == '__main__':
    ...
    'outputs/dataset_images_srdense/combine_all_degradation_fix_dataset/srdense/srdense_combine_all_degradation_fix_dataset/checkpoints/z_axis/factor_2/epoch_450_f_2.pth',
    ...
    model_names = ['gaussian model', 'hanning model', 'hamming model', 'bicubic model', 'mean model', 'median model', 'combine model']
    ...
    input_score = [31.897264001188802, 0.0006460611475631595, 0.8494298939461702, 0.08729839324951172, 0.03150997683405876] #copy from the terminal after running code once
