...
from utils.prepare_test_set_image import crop_pad_kspace, downsample_gaussian, prepare_image_fourier,prepare_image_gaussian
...
# label_path ='test_set/test_label/'
label_path ='resolution_dataset25_old/z_axis/label/test/'
...
# correct version
def predict_downsample_edges(opt,model,label_path,factor,device,psnr,ssim):

    ref = cv2.imread(label_path)
    ref = cv2.cvtColor(ref, cv2.COLOR_BGR2GRAY).astype(np.float32)
    ref = ref/255.

    print('range of ref:', ref.min().item(), ref.max().item())
    degraded = crop_pad_kspace(ref,pad=True,factor=factor).astype('float32')
    downsample_factor = factor+factor
    downsample = crop_pad_kspace(ref,pad=True,factor=downsample_factor).astype('float32')
    print('range of lr image:', degraded.min().item(), degraded.max().item())
    print('range of downsample image:', downsample.min().item(), downsample.max().item())
    ...
    pre_edges = model(input_edges)
    pre=pre_edges+degraded
    # print('RANGE OF PREDICTION', pre.min().item(), pre.max().item())
    ...
    # i_edge = input_edges
    # i_edge = i_edge.squeeze().detach().cpu()
    # min= i_edge.min().item()
    # max = i_edge.max().item()
    # hist = torch.histc(i_edge, bins = bins, min = min, max =max)
    # x = torch.linspace(min,max, steps=bins)
    # plt.bar(x, hist, align='center')
    # plt.xlabel('Bins')
    # plt.ylabel('Frequency')
    # plt.savefig(f'{file}_in_edg.png')
    # plt.show()
    # print('saved input edges')
...
def save_results_edges(model,path,opt):
    ...
    for file in os.listdir(path):
        ...
        if opt.edge_type in ['downsample']:
            ...
            ref, degraded, output, input_edges, output_edges, label_edges, intial_baseline, scores = predict_downsample_edges(opt,model,label_path='{}/{}'.format(path,file),factor=opt.factor,device=opt.device,psnr=opt.psnr,ssim=opt.ssim)
...
if __name__ == '__main__':
    ...
    if opt.edges:
        ...
        save_results_edges(model,label_path,opt)
    
    else:
        save_results(model,label_path,opt)
