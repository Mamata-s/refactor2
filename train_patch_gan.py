...
from utils.general import save_configuration
...
def train(opt,model,train_dataloader,eval_dataloader,wandb=None):
    ...
    for epoch in range(opt.num_epochs):
        adjust_learning_rate(model.opt_G, epoch,opt)
        adjust_learning_rate(model.opt_D, epoch,opt)
        ...
        apply_model(model.net_G,epoch,opt,addition=False)
...
if __name__ == "__main__":
    ...
    parser.add_argument('--config', help="configuration file *.yml", type=str, required=False, default='yaml/patch_gan.yaml')
    sys.argv = ['-f']
    ...
    if opt.wandb:
        ...
        wandb.watch(model.net_G,log="all",log_freq=1)
