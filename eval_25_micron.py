...
def evaluate_model(opt):
    ...
    for key in opt.dictionary: 
        image_name = opt.dictionary[key]
        image_path = os.path.join(opt.image_path,image_name)
        label_path = os.path.join(opt.label_path, key)
...
if __name__ == '__main__':
    ...
    sigmas = [50,75,100,125,150]
    sigmas = ['bicubic','combined', 'kspace','simulated degradation', 'real degraded' ]
    checkpoints = []
    for sigma in  sigmas:
    ...
    checkpoints = ['outputs/dataset_images/bicubic_dataset25/srdense/bicubic_up_and_down_factor_2/checkpoints/z_axis/factor_2/epoch_500_f_2.pth',
    ...
    # image_path_pad = 'upsample/combine/upsample_image'
    # image_path_sigma = ['upsample/combine/upsample_bicubic', 'upsample/combine/upsample_image','upsample/combine/upsample_image','upsample/combine/upsample_image','upsample/combine/upsample_image']
    # label_path = 'upsample/combine/hr_25/z_axis'

    annotations = 'upsample/f1_160/annotation_hr1_dict.pkl'
    image_path_pad = 'upsample/f1_160/upsample_bicubic'
    image_path_sigma = ['upsample/f1_160/upsample_image_sigma_50', 'upsample/f1_160/upsample_image_sigma_75',
    'upsample/f1_160/upsample_image_sigma_100','upsample/f1_160/upsample_image_sigma_125',
    'upsample/f1_160/upsample_image_sigma_150']
    label_path = 'upsample/f1_160/hr_25/z_axis'
    

    #model name to save yaml file
    model_name = [ 'sigma_50_model', 'sigma_75_model', 'sigma_100_model','sigma_125_model','sigma_150_model' ]
    # initial name to save yaml file
    initial_name = ['sigma_50_initial', 'sigma_75_initial', 'sigma_100_initial','sigma_125_initial', 'sigma_150_initial']


    #storing result dictionary in list
    initial = []
    model = []
    ...
    for index,checkpoint in enumerate(checkpoints):
        ...
        model.to(device)
        model.eval()
        opt.dictionary = read_dictionary(annotations)
        opt.model=model

        opt.model_name = model_name[index]
        opt.initial_name = initial_name[index]
        # opt.image_path = image_path_pad
        # opt.label_path = label_path

        # initial_pad, model_pad = evaluate_model(opt)  # each dictionary of list with keys psnr,ssim,mse,nrmse,hfen
        # initial_pad_list.append(initial_pad) #append dictionary for each sigma to a list
        # model_pad_list.append(model_pad)
        ...
        initial_sigma_list.append(initial_sigma)
        print("Evaluating for gaussian data")
    ...
    # print("Length of list", len(initial_pad_list))
    # print(initial_pad_list[0]['psnr'])
    # plot_boxplot(initial_pad_list=initial_pad_list,initial_sigma_list=initial_sigma_list, model_pad_list=model_pad_list,model_sigma_list=model_sigma_list)    
